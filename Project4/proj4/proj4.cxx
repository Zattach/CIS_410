                              ...
                              const float *Y, const float *F, float *rv)
{
    ...
    int vertexId[4];
    vertexId[0] = 2 * GetPointIndex(firstVertex, dims);
    firstVertex[0]++;
    vertexId[1] = 2 * GetPointIndex(firstVertex, dims);
    firstVertex[0]--;    firstVertex[1]++;
    vertexId[2] = 2 * GetPointIndex(firstVertex, dims);
    firstVertex[0]++;
    vertexId[3] = 2 * GetPointIndex(firstVertex, dims);
    firstVertex[0]--;    firstVertex[1]--;
    ...
    float interpolatedLowerXValue = F[vertexId[0]] + (tValueX * (F[vertexId[1]] - F[vertexId[0]]));
    float interpolatedLowerYValue = F[vertexId[0] + 1] + (tValueX * (F[vertexId[1] + 1] - F[vertexId[0] + 1]));
    float interpolatedHigherXValue = F[vertexId[2]] + (tValueX * (F[vertexId[3]] - F[vertexId[2]]));
    float interpolatedHigherYValue = F[vertexId[2] + 1] + (tValueX * (F[vertexId[3] + 1] - F[vertexId[2] + 1]));

    // Calculate interpolated F-values for final vertex
    rv[0] = interpolatedLowerXValue + (tValueY * (interpolatedHigherXValue - interpolatedLowerXValue));
    rv[1] = interpolatedLowerYValue + (tValueY * (interpolatedHigherYValue - interpolatedLowerYValue));
...
{
    // Initialize vec and prevPt
    float vec[2];
    float prevPt[2];

    // Populate vec[] at seed
    EvaluateVectorFieldAtLocation(pt, dims, X, Y, F, vec);

    // 0-out arrays if there is no initial movement
    if(vec[0] == 0 && vec[1] == 0){
        for(int i = 0; i <= nsteps; i++){
            speeds[i] = output_locations[2*i] = output_locations[2*i+1] = 0;
        }
        return;
    }

    // Solve speed anc location at seed
    speeds[0] = sqrt(pow(vec[0], 2) + pow(vec[1], 2));
    output_locations[0] = pt[0];
    output_locations[1] = pt[1];

    for(int i = 1; i <= nsteps; i++){
        // Populate prevPt with the previous point location
        prevPt[0] = output_locations[2*(i-1)];
        prevPt[1] = output_locations[2*(i-1)+1];

        // Populate vec of the previous point
        EvaluateVectorFieldAtLocation(prevPt, dims, X, Y, F, vec);

        // Solve speed and location at step i
        speeds[i] = sqrt(pow(vec[0], 2) + pow(vec[1], 2));
        output_locations[2*i] = prevPt[0] + (h * vec[0]);
        output_locations[2*i+1] = prevPt[1] + (h * vec[1]);
    }
}
...
{
    // Initialize arcLength and differenceVector
    float arcLength = 0.;
    float differenceVector[2];

    // Calculate distance between 2 points and add to arcLength (looped)
    for(int i = 0; i < nlocations-1; i++){
        differenceVector[0] = output_locations[2*(i+1)] - output_locations[2*i];
        differenceVector[1] = output_locations[2*(i+1)+1] - output_locations[2*i+1];
        arcLength += sqrt(pow(differenceVector[0], 2) + pow(differenceVector[1], 2));
    }
